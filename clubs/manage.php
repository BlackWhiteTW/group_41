...
try {
    ...
    if (empty($clubs)) {
        ...
        $stmt = $pdo->prepare('SELECT c.*, u.username AS owner_name, u.email AS owner_email FROM clubs c JOIN users u ON u.id = c.owner_user_id WHERE c.id = :id LIMIT 1');
        ...
        if (!$club) {
            ...
            $ann_stmt = $pdo->prepare('SELECT a.title, a.content, a.created_at, u.username FROM club_announcements a JOIN users u ON u.id = a.user_id WHERE a.club_id = :c ORDER BY a.created_at DESC LIMIT 10');
                            ...
                            <div class="panel" style="padding: 20px">
                                ...
                                <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">
                                    ...
                                    <?php if (($club['visibility'] ?? 'public') === 'private'): ?>
                                        <span class="pill" style="background:#f3e8fa;color:#7a3b9e">私人社團</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="panel" style="padding: 20px">
                                ...
                                <div style="padding:10px 0;border-bottom:1px solid #e4efe8">
                                    ...
                                    <?php if (!empty($ann['content'])): ?>
                                        <p class="muted" style="margin:4px 0;font-size:0.88rem;white-space:pre-wrap"><?php echo htmlspecialchars($ann['content']); ?></p>
                                    <?php endif; ?>
                                    <span class="meta" style="margin-top:2px"><?php echo htmlspecialchars($ann['username']); ?> · <?php echo date('m/d H:i', strtotime($ann['created_at'])); ?></span>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php endif; ?>
