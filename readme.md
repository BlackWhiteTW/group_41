...
| `functions.php` | 共用工具（目標社團解析、上傳白名單 / 內容驗證、`ensure_uploads_dir()`） |
| `mail.php` | 郵件發送（PHP `mail()` / SMTP）與密碼重設信件內容 |
...
### uploads/ — 上傳檔案
執行時由 `ensure_uploads_dir()`（`includes/functions.php`）自動建立，並補上 `.htaccess`（`Require all denied`）禁止直接存取，因此不需納入版本控制。

## 資料庫結構（13 張表）
1. `users` — 使用者帳號（含 `remember_token_hash`，記住我 Token 已合併於此）
2. `clubs` — 社團基本資料
...
8. `answers` — 答案明細
9. `password_resets` — 密碼重設
10. `club_invitations` — 社團邀請
11. `club_join_requests` — 加入申請
12. `club_announcements` — 社團公告
13. `club_activity_log` — 活動稽核日誌
